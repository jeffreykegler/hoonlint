...
sub getCram {
	...
	say STDERR "Before indent <= 0 loop: indent=$indent; nextNlPos=$nextNlPos";
    if ($indent <= 0) {
	say STDERR "Top of indent <= 0 loop: indent=$indent; nextNlPos=$nextNlPos";
	...
	say STDERR join ' ', __FILE__, __LINE__, "Returning from getCram at $nextNlPos";
	...
	say STDERR join ' ', __FILE__, __LINE__, "Returning from getCram at $inputLength";
	...
	say STDERR "Before LINE loop: indent=$indent; nextNlPos=$nextNlPos";
    LINE: while ($nextNlPos >= 0) {
	say STDERR "LINE: indent=$indent; nextNlPos=$nextNlPos";
	pos ${$input} = $nextNlPos + 1;
	say STDERR join ' ', __FILE__, __LINE__, qq{Pos set to: }, substr(${$input}, $nextNlPos+1, 20);
	if ( ${$input} =~ m/\G $indentString [ ]* == [\n]/xms ) {
	    my $terminatorStartPos = $LAST_MATCH_START[0];
	    my $terminatorEndPos   = $LAST_MATCH_END[0];
	    my $terminatorLength = $terminatorEndPos - $terminatorStartPos;
	    say STDERR qq{TISTIS found: }, substr(${$input}, $terminatorStartPos, $terminatorEndPos-$terminatorStartPos);
	    my $value              = substr( ${$input}, $valueStartPos,
		$terminatorStartPos - $valueStartPos );
	    # Continue parsing after TISTIS?  Or before?
	    say STDERR join ' ', __FILE__, __LINE__, "Returning from getCram at $terminatorEndPos";
	    return \$value, $nextNlPos;
	}
	...
	say STDERR qq{indentString="$indentString"};
	say STDERR qq{this indent: "}, (substr ${$input}, $nextNlPos+1, $indent), qq{"};
	...
	say STDERR join ' ', __FILE__, __LINE__, "Returning premature EOF from getCram";
    return;
}
  ...
  READ:
	...
	# TODO: tripeDoubleQuote must allow sump(5d)
        ...
        if ( $eventName eq '^CRAM' ) {
            my $value_ref;
	say STDERR join ' ', __FILE__, __LINE__, "About to call getCram(): this_pos=$this_pos ; input_length=$input_length";
            ( $value_ref, $resume_pos )
	      = getCram( $input, $this_pos );
	say STDERR join ' ', __FILE__, __LINE__, "About to call getCram(): resume_pos=$resume_pos; this_pos=$this_pos ; input_length=$input_length";
	    if (not $value_ref) {
	...
	say STDERR join ' ', __FILE__, __LINE__, "resume_pos=$resume_pos; this_pos=$this_pos ; input_length=$input_length";
        my $ok = eval {
	say STDERR join ' ', __FILE__, __LINE__, "resume_pos=$resume_pos; this_pos=$this_pos ; input_length=$input_length";
	use Devel::Peek;
	$resume_pos = int($resume_pos) if defined $resume_pos;
	if (defined $resume_pos) { Dump $resume_pos } else { say STDERR "resume pos not defined"; }
