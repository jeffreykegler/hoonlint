...
sub is_0Jogging {
	...
	my $instance =  $policy->{lint};
      ...
      CHILD: for my $childIX ( 0 .. $#$children ) {
            ...
            my $side = $instance->joggingSide( $child, $runeColumn );
            my $bodyAlignment = $instance->joggingBodyAlignment( $child, $runeColumn );
...
# say join " ", __FILE__, __LINE__, "set rune column:", $context->{jogRuneColumn} ;
        my $chessSide = $context->{chessSide};
        return $instance->checkQueensideJog( $node, $context )
          if $chessSide eq 'queenside';
        return $instance->checkKingsideJog( $node, $context );
    };

    sub isBackdented {
        my ( $policy, $indents, $baseIndent ) = @_;
        my @mistakes = ();
    ...
    # if here, gapiness > 0
    {
      ...
      TYPE_INDENT: {
            ...
            if ( $tallNoteRule->{$lhsName} ) {
                $mistakes = $policy->isBackdented( \@gapIndents, $noteIndent );
                last TYPE_INDENT if @{$mistakes};
            ...
            if ( $tallLuslusRule->{$lhsName} ) {
                $mistakes = $policy->isLuslusStyle( \@gapIndents );
                last TYPE_INDENT if @{$mistakes};
